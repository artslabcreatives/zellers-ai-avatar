...
export function VotePageContent() {
	...
	const fetchPosts = useCallback(async (f: Filter, p: number, searchTerm: string = '', pageSize: number = 10) => {
		...
		setError("");
		try {
			// Build URL with search and limit parameters
			const searchParam = searchTerm.trim() ? `&search=${encodeURIComponent(searchTerm.trim())}` : '';
			const limitParam = pageSize === 0 ? '&limit=all' : `&limit=${pageSize}`;
			const url =
				f === "all" ? `/api/posts?page=${p}${limitParam}${searchParam}` :
					f === "kumara" ? `/api/posts/male?page=${p}${limitParam}${searchParam}` :
						f === "kumariya" ? `/api/posts/female?page=${p}${limitParam}${searchParam}` :
							`/api/posts/top/mixed`;
			...
			// Posts come sorted by rank from the backend (highest votes = rank 1)
			setPosts(data.posts ?? []);
	...
	// Fetch and highlight a specific post from share link
	useEffect(() => {
		if (!highlightPostId || highlightedPostData) return;

		// Fetch the specific post
		fetch(`/api/posts/${highlightPostId}`)
			.then((r) => r.json())
			.then((data) => {
				if (data.success && data.postDetails) {
					...
					const post: Post = {
						...
						postNumber: data.postDetails.postNumber,
						displayName: data.postDetails.displayName,
						gender: data.postDetails.gender,
						flavor: data.postDetails.flavor,
						voteCount: data.postDetails.voteCount,
						rank: data.postDetails.rank,
						imageUrl: data.postDetails.imageUrl,
						shareUrl: data.postDetails.shareUrl,
					};
					setHighlightedPostData(post);
					setHighlightedPost(highlightPostId);
					setShowShareModal(true); // Show the modal immediately
					// Remove highlight after 8 seconds
					setTimeout(() => setHighlightedPost(null), 8000);
				}
			})
			.catch((error) => {
				console.error('Failed to fetch highlighted post:', error);
			});
